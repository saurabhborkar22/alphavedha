"""Tests for the regime-aware exposure overlay (always-on)."""
...
def test_no_market_features_still_caps_kelly() -> None:
    ov = RegimeOverlay(kelly_cap=0.25)
    kelly, tradeable, warning = apply_regime_overlay(ov, 1.0, 1, True, None)
    assert kelly == 0.25
...
def test_downtrend_suppresses_long_and_cuts_size() -> None:
    ov = RegimeOverlay(kelly_cap=0.25, downtrend_size_mult=0.3)
    kelly, tradeable, warning = apply_regime_overlay(ov, 1.0, 1, True, _market(-0.01))
    assert tradeable is False
    assert warning == "regime_overlay_long_suppressed_downtrend"
    assert kelly == 0.25 * 0.3


def test_downtrend_keeps_shorts_tradeable() -> None:
    ov = RegimeOverlay(kelly_cap=0.25, downtrend_size_mult=0.3)
    kelly, tradeable, warning = apply_regime_overlay(ov, 0.2, -1, True, _market(-0.01))
    assert tradeable is True
    assert warning is None
    assert kelly == 0.2 * 0.3
...
def test_load_overlay_always_returns_instance() -> None:
    ov = _load_regime_overlay()
    assert isinstance(ov, RegimeOverlay)
    assert ov.kelly_cap == 0.25
    assert ov.suppress_longs_in_downtrend is True


def test_load_overlay_from_env(monkeypatch) -> None:
    monkeypatch.setenv("ALPHAVEDHA_REGIME_OVERLAY_KELLY_CAP", "0.15")
    assert _load_regime_overlay().kelly_cap == 0.15
